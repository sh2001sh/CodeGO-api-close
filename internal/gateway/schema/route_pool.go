...
type RoutePoolMember struct {
	...
	// FaultDomain groups channels that share a transient upstream dependency.
	// An empty value falls back to the channel type and upstream host.
	FaultDomain string         `json:"fault_domain" gorm:"size:128;not null;default:''"`
	Enabled     bool           `json:"enabled" gorm:"not null;default:true;index"`
