...
var defaultBillingExpr = map[string]string{
	...
	"gpt-5.6-terra": `len <= 272000 ? tier("short_context", p * 2 + c * 12 + cr * 0.25 + cc * 2.5) : tier("long_context", p * 4 + c * 18 + cr * 0.5 + cc * 5)`,
	"gpt-5.6-luna":  `len <= 272000 ? tier("short_context", p * 0.2 + c * 1.2 + cr * 0.1 + cc * 0.25) : tier("long_context", p * 0.4 + c * 1.8 + cr * 0.2 + cc * 0.5)`,
