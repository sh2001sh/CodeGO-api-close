...
func RegisterUserRoutes(apiRouter *gin.RouterGroup, anonymousRequestBodyLimit gin.HandlerFunc) {
	...
	userRoute := apiRouter.Group("/user")
	{
		...
		userRoute.GET("/logout", middleware.DisableCache(), Logout)
		...
		{
			selfRoute.GET("/self/groups", middleware.DisableCache(), gatewayhttp.GetUserGroups)
			selfRoute.GET("/self/group-status", middleware.DisableCache(), gatewayhttp.GetUserGroupStatus)
			selfRoute.GET("/self/group-overview", middleware.DisableCache(), gatewayhttp.GetUserGroupOverview)
			selfRoute.GET("/self", middleware.DisableCache(), GetUserSelf)
...
func RegisterDesktopRoutes(apiRouter *gin.RouterGroup) {
	...
	desktopRoute.Use(middleware.DesktopAuth())
	{
		...
		desktopRoute.GET("/group-status", middleware.RequireDesktopScope(identitydomain.DesktopScopeAccountRead), middleware.DisableCache(), gatewayhttp.GetUserGroupStatus)
