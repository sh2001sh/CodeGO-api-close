...
func TestOwnerUsageJSONFields(t *testing.T) {
	data, err := json.Marshal(OwnerUserUsageItem{RequestCount: 10, SuccessCount: 9, FailedCount: 1, AvgLatencyMs: 200, AvgTTFTMs: 100, TotalConsumerAmount: 60, TotalSettlementGrossAmount: 60, TotalOwnerIncome: 57})
	require.NoError(t, err)
	var fields map[string]any
	require.NoError(t, json.Unmarshal(data, &fields))
	for key, value := range map[string]float64{"request_count": 10, "success_count": 9, "failed_count": 1, "avg_latency_ms": 200, "avg_ttft_ms": 100, "total_consumer_amount": 60, "total_settlement_gross_amount": 60, "total_owner_income": 57} {
...
func TestOwnerUsageNormalizesAndAggregatesChannelUsers(t *testing.T) {
	...
	for _, s := range []marketplaceschema.Settlement{
		...
		{RequestID: "old-subscription", GroupID: "g1", OwnerUserID: 10, ConsumerUserID: 21, ConsumerAmount: 605, OwnerNetAmount: 57, BillingSource: "subscription"},
	...
	require.Equal(t, int64(121), byUser["21"].TotalSettlementGrossAmount)
	require.Equal(t, int64(121), byUser["21"].TotalConsumerAmount)
	require.Equal(t, int64(181), result["summary"].(map[string]any)["consumer_amount"])
	require.Equal(t, int64(4), result["summary"].(map[string]any)["total_requests"])
	paged, err := ListOwnerChannelUserUsage(10, OwnerUserUsageQuery{ChannelID: "c1", PageSize: 1, Sort: "amount", Direction: "desc"})
	require.NoError(t, err)
	require.Len(t, paged["items"].([]OwnerUserUsageItem), 1)
	require.EqualValues(t, 2, paged["total"])
	require.EqualValues(t, 2, paged["summary"].(map[string]any)["total_users"])
	require.Equal(t, int64(181), paged["summary"].(map[string]any)["consumer_amount"])
	require.Equal(t, "21", paged["items"].([]OwnerUserUsageItem)[0].UserID)
