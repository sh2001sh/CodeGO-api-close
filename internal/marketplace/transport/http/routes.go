...
func RegisterMarketplaceRoutes(apiRouter *gin.RouterGroup) {
	...
	publicMarketplaceRoute.Use(middleware.HeaderNavModulePublicOrUserAuth("pricing"))
	{
		...
		publicMarketplaceRoute.GET("/models", ListModels)
	...
	marketplaceRoute.Use(middleware.UserAuth())
	{
		...
		marketplaceRoute.PUT("/auto-route-pool", middleware.RoutePoolMutationRateLimit(), UpdateAutoRoutePool)
		marketplaceRoute.GET("/route-pools", ListRoutePools)
		marketplaceRoute.POST("/route-pools", middleware.RoutePoolMutationRateLimit(), CreateRoutePool)
		marketplaceRoute.GET("/route-pools/:id", GetRoutePool)
		marketplaceRoute.PUT("/route-pools/:id", middleware.RoutePoolMutationRateLimit(), UpdateRoutePool)
		marketplaceRoute.POST("/route-pools/:id/auto-build/run", middleware.RoutePoolMutationRateLimit(), RunRoutePoolAutoBuild)
		marketplaceRoute.DELETE("/route-pools/:id", middleware.RoutePoolMutationRateLimit(), DeleteRoutePool)
		marketplaceRoute.POST("/route-pools/:id/bind-token", middleware.RoutePoolMutationRateLimit(), BindRoutePoolToken)
		...
		marketplaceRoute.GET("/channels/mine/logs/export", middleware.DownloadRateLimit(), ExportMyUsageLogs)
		...
		marketplaceRoute.GET("/security-audit/events", ListOwnerSecurityAuditEvents)
		marketplaceRoute.GET("/security-audit/events/export", middleware.DownloadRateLimit(), ExportOwnerSecurityAuditEvents)
		marketplaceRoute.PATCH("/security-audit/events/:id", UpdateOwnerSecurityAuditEvent)
