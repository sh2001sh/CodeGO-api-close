      ...
      if (props.channel) {
        await update.mutateAsync({
          ...
          values: {
            ...
            multiplier_card_supported: values.multiplier_card_supported,
            multiplier_card_user_enabled: values.multiplier_card_user_enabled,
        ...
        toast.success(
          t(
            '渠道已提交，请先点击“测试连通性”确认可用；GPT-5.6 一致性检测仅用于核对模型映射'
          )
        )
    ...
    <form onSubmit={submit}>
      ...
      <div className='border-border bg-muted/15 flex justify-end border-t px-4 py-4 sm:px-5'>
        <Button type='submit' disabled={pending}>
          ...
          {editing ? t('保存修改（留空保持原值）') : t('提交并执行必做校验')}
        </Button>
      </div>
    </form>
  )
