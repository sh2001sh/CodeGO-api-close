...
function group(
  input: Partial<MarketplaceGroup> &
    Pick<
      MarketplaceGroup,
      | 'id'
      | 'system_display_name'
  ...
  return {
    ...
    model_verification_results: input.models.map((model) => ({
      model,
      status: 'passed',
      listed: true,
      latency_ms: input.avg_latency_ms ?? 800,
      tested_at: new Date(now - 1800_000).toISOString(),
    })),
    ...
    recent_request_series: Array.from({ length: 12 }, (_, index) => ({
      ts: now - (11 - index) * 300_000,
      success_rate: 0.94 + (index % 4) * 0.01,
      request_count: 18 + index,
    })),
...
export const MOCK_MARKETPLACE_GROUPS: MarketplaceGroup[] = [
  group({
    id: 'mock-official-fast',
    system_display_name: '官方 · GPT-5.2 Fast',
  ...
  group({
    id: 'mock-community-steady',
    system_display_name: '社区 · 稳定编程组',
    source_label: '社区贡献',
    provider_type: 'OpenAI Compatible',
    models: ['gpt-5.2', 'claude-3-7-sonnet'],
    multiplier: 1.15,
    rank: 2,
    score: 93,
    avg_ttft_ms: 360,
  ...
  group({
    id: 'mock-cloud-balanced',
    system_display_name: '云端 · Balanced',
    source_label: '第三方市场',
  ...
  group({
    id: 'mock-lab-low-cost',
    system_display_name: '实验室 · 低倍率',
    source_label: '第三方市场',
  ...
  group({
    id: 'mock-research',
    system_display_name: '研究组 · Long Context',
    source_label: '第三方市场',
...
export const MOCK_AUTO_ROUTE_POOL: MarketplaceAutoRoutePool = {
  token_group: 'auto',
  selected_count: 2,
  items: MOCK_MARKETPLACE_GROUPS.slice(0, 2).map((item, index) => ({
    group_id: item.id,
    source_type:
      item.source_type === 'official' ? 'official' : 'marketplace_user',
    public_slug: item.public_slug,
    system_display_name: item.system_display_name,
    source_label: item.source_label,
    lifecycle_status: item.lifecycle_status,
    multiplier: item.multiplier,
    availability: 0.99 - index * 0.01,
    success_rate: item.success_rate,
    cache_hit_rate: item.cache_hit_rate,
    avg_ttft_ms: item.avg_ttft_ms,
    avg_latency_ms: item.avg_latency_ms,
    latest_request_status: item.latest_request_status,
    metrics_available: true,
    route_score: item.score,
    observing: false,
    request_count: item.request_count,
    models: item.models,
    avg_consumer_amount: item.avg_consumer_amount,
    avg_consumer_amount_by_model: item.avg_consumer_amount_by_model,
    selected: true,
    priority: index + 1,
  })),
  config: {
