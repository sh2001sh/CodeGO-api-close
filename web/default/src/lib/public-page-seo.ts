...
export const publicPageSeoEntries: PublicPageSeoEntry[] = [
  {
    ...
    sections: [
      {
        heading: '先按任务类型筛，而并非先按模型名筛',
        paragraphs: [
          '真正高频使用 AI 编程的开发者，往往并非一开始就锁定某个模型，而是先判断当前任务更偏向信息整理、轻量改写、复杂推理，还是长上下文代码协作。模型广场的价值，就是把这些判断动作提前做掉。',
      ...
      },
      {
        ...
        paragraphs: [
          'Bing 和用户都会更偏好信息明确的页面。这里会直接展示输入价、输出价、缓存价和可见模型数，而并非只写一句笼统的低价或稳定。',
          '对长期使用者来说，真正重要的是把模型能力、调用成本和任务价值对齐，而并非每次都从零开始比较。',
        ],
      },
      {
        heading: '模型广场并非终点，而是下一步决策入口',
        paragraphs: [
  ...
  },
  {
    ...
    eyebrow: '使用指南',
    intro:
      '这并非只讲一个按钮怎么点的说明页，而是把注册、登录、API Key、脚本下载、套餐、盲盒、钱包和宠物体系串成一条完整路径，让新用户可以顺着站点结构真正开始使用。',
    sections: [
      {
        heading: '从注册到本地工具配置，要有完整链路',
        paragraphs: [
          '搜索教程类关键词的用户，通常并非在找一个抽象介绍，而是在找“接下来应该做什么”。因此这页把注册、登录、控制台、密钥创建和脚本下载按实际顺序展开。',
      ...
      },
      {
        ...
        paragraphs: [
          'Code Go 的公开页面不只是展示模型，也承接套餐订阅、额度管理、盲盒活动和宠物成长机制。教程页会把这些功能入口和使用场景解释清楚，而并非留成不可理解的站内术语。',
      ...
      },
      {
        ...
        paragraphs: [
          '很多人会直接搜索“Codex 配置”“API Key”“使用说明”这类词进入站点，因此这页需要在初始 HTML 里就具备清晰标题、描述、H1 和足够正文，而并非等前端加载完成后再补。',
  ...
  },
  {
    ...
    eyebrow: 'FAQ',
    intro:
      'FAQ 页并非为了堆问题数量，而是为了快速回答用户最先关心的事情：Code Go 是什么、适合谁、和普通 AI API 平台有什么不同，以及为什么它强调长期使用与记录积累。',
    sections: [
      {
  ...
  },
  {
    ...
    eyebrow: '关于我们',
    intro:
      '关于页的目标并非一句空泛品牌文案，而是解释 Code Go 为什么存在、适合什么样的开发者，以及为什么它把“长期使用感”和“持续积累”放在公开表达最前面。',
    sections: [
      {
        heading: 'Code Go 并非只解决一次调用',
        paragraphs: [
          '对于高频使用 Codex、Claude Code 或多模型协作的开发者来说，真正重要的问题并非能不能调用某个模型，而是能不能把模型纳入长期工作流。',
      ...
      },
      {
        ...
        paragraphs: [
          '如果首页、教程页、模型页和常见问题页都围绕长期 AI 编程展开，那么关于页也必须与这条主线保持一致，而并非变成泛化介绍。',
    ...
    sections: [
      {
        ...
        paragraphs: [
          'Code Go Desktop 的作用并非单纯下载一个壳，而是把网站账号、浏览器授权和本地工具配置连接起来，让 Codex、Claude Code、Gemini CLI 等工具可以在一处统一管理。',
      ...
      },
      {
        heading: '下载页也是站内分流节点',
        paragraphs: [
          '用户完成下载后，下一步往往是打开 token console、查看 FAQ，或继续回到模型页和教程页。因此下载页本身也应该具备良好的内部链接和足够正文，而并非孤立页面。',
    ...
    sections: [
      {
        ...
        paragraphs: [
          'Code Go 会涉及账号、调用、支付、偏好设置和必要的安全日志，因此隐私页需要明确这些信息分别用在什么地方，而并非只给出泛化条款。',
      ...
      },
      {
        ...
        paragraphs: [
          ...
          '这类法律页虽然并非营销页，但同样需要独立 title、description、H1 和可见正文。',
        ],
      },
      {
    ...
    sections: [
      {
        ...
        paragraphs: [
          'Code Go 既有模型调用，也有套餐、盲盒、宠物和额度体系，所以协议页必须覆盖这些真实功能，而并非只保留笼统的互联网服务条款。',
